...
// 		List boardList = bd.getBoardList();   
		...
		if(count != 0){
			boardList = bd.getBoardList(startRow, pageSize, search); // paging	
		}
	%>
	...
	<div id="wrap">
		...
		<article>
			...
			%>			<a href="board_list.jsp?pageNum=<%=startPage-pageBlock%>&search=<%=search%>">[이전]</a><%
					}
					
					for(int i = startPage; i <= endPage; i++){
			%>			<a href="board_list.jsp?pageNum=<%=i%>&search=<%=search%>"><%=i %></a><%
					}	
					
					// [다음]
							
					if(endPage < pageCount){
			%>			<a href="board_list.jsp?pageNum=<%=startPage+pageBlock%>&search=<%=search%>">[다음]</a><%
					}
				}
