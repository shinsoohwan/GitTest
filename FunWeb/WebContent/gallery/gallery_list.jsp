...
// 		List GalleryList = gd.getGalleryList();   
		...
		if(count != 0){
			galleryList = gd.getGalleryList(startRow, pageSize); // paging	
		}
	%>
...
// 				int gY = 2;	// pageSize / gX
			...
			%>			<a href="gallery_list.jsp?pageNum=<%=startPage-pageBlock%>">[이전]</a><%
					}
					
					for(int i = startPage; i <= endPage; i++){
			%>			<a href="gallery_list.jsp?pageNum=<%=i%>"><%=i %></a><%
					}	
					
					// [다음]
							
					if(endPage < pageCount){
			%>			<a href="gallery_list.jsp?pageNum=<%=startPage+pageBlock%>">[다음]</a>
