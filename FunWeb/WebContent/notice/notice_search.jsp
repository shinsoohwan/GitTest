...
<body>
	<%
		...
		if(count != 0){
			noticeList = nd.getNoticeList(startRow, pageSize, search); // paging	
		}
	%>
	...
	<div id="wrap">
		...
		<article>
			...
			%>			<a href="notice_search.jsp?pageNum=<%=startPage-pageBlock%>&search=<%=search%>">[이전]</a><%
					}
					
					for(int i = startPage; i <= endPage; i++){
			%>			<a href="notice_search.jsp?pageNum=<%=i%>&search=<%=search%>"><%=i %></a><%
					}	
					
					// [다음]
							
					if(endPage < pageCount){
			%>			<a href="notice_search.jsp?pageNum=<%=startPage+pageBlock%>&search=<%=search%>">[다음]</a><%
					}
				}
